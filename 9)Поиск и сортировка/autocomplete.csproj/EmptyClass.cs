...
namespace Manipulation
{
    public static class ManipulatorTask
    {
        ...
        {
            var wristAxisX = y + Math.Sin(Math.PI - angle) * Manipulator.Palm;
            var wristAxisY = x + Math.Cos(Math.PI - angle) * Manipulator.Palm;
            var length = Math.Sqrt(wristAxisX * wristAxisX + wristAxisY * wristAxisY);
