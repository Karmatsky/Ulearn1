...
namespace Manipulation
{
    public static class ManipulatorTask
    {
        public static double[] MoveManipulatorTo(double x, double y, double angle)
        {
            ...
            var length = Math.Sqrt(wristAxisY * wristAxisY + wristAxisX * wristAxisX);
            var elbow = TriangleTask.GetABAngle(Manipulator.UpperArm, Manipulator.Forearm, length);
            var shoulder = TriangleTask.GetABAngle(Manipulator.UpperArm, length, Manipulator.Forearm) + Math.Atan2(wristAxisY, wristAxisX);
